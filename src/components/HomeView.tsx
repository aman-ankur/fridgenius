...
export default function HomeView({
  ...
  const forYouCard = (
    <>
      <section className="grid grid-cols-2 gap-2.5" data-testid="for-you-card">
        <button onClick={onOpenMealIdeas || (allMeals.length >= 3 ? loadRecommendations : () => {})} className="group min-w-0 rounded-[20px] border border-accent/20 bg-gradient-to-br from-[#E8F5E0] via-white to-white p-3.5 text-left shadow-[0_6px_18px_rgba(71,101,64,0.06)] transition-shadow hover:shadow-[0_8px_22px_rgba(71,101,64,0.1)] active:scale-[0.99]">
          <div className="mb-3 flex h-8 w-8 items-center justify-center rounded-xl bg-white text-accent shadow-sm ring-1 ring-accent/10"><Sparkles className="h-4 w-4" /></div>
          <p className="text-[13px] font-extrabold leading-tight text-foreground">What should I eat?</p>
          <p className="mt-1 min-h-[28px] text-[10px] leading-snug text-muted">{allMeals.length >= 3 ? `Personalized ideas for ${inferredMealType}` : `Log ${3 - allMeals.length} more meal${3 - allMeals.length === 1 ? "" : "s"} to unlock`}</p>
          <span className="mt-2 block text-sm font-extrabold text-accent-dim">{recommendationLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />}</span>
        </button>
        <button onClick={onOpenHealthPlaybook} className="group min-w-0 rounded-[20px] border border-violet-200/70 bg-gradient-to-br from-[#F4F0FF] via-white to-white p-3.5 text-left shadow-[0_6px_18px_rgba(111,82,170,0.05)] transition-shadow hover:shadow-[0_8px_22px_rgba(111,82,170,0.09)] active:scale-[0.99]">
          <div className="mb-3 flex h-8 w-8 items-center justify-center rounded-xl bg-white text-accent-dim shadow-sm ring-1 ring-accent/10"><ShieldCheck className="h-4 w-4" /></div>
          <p className="text-[13px] font-extrabold leading-tight text-foreground">Health Playbook</p>
          <p className="mt-1 min-h-[28px] text-[10px] leading-snug text-muted">{(() => { const s = buildHealthPlaybook(allMeals, healthProfile); return `${s.status === "unlocked" ? "Ready to explore" : s.status === "learning" ? "Learning your patterns" : s.status === "sparse" ? "Limited edition" : "Early preview"} · ${s.daysLogged}/7 days`; })()}</p>
          <span className="mt-2 block text-sm font-extrabold text-accent-dim"><ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" /></span>
        </button>
        {latestAnalysis && Math.floor((Date.now() - new Date(latestAnalysis.generatedAt).getTime()) / (1000 * 60 * 60 * 24)) <= 7 && (() => { const scoreInfo = SCORE_LABELS[latestAnalysis.report.score]; return <button onClick={onViewAnalysis} className="group col-span-2 flex items-center gap-3 rounded-2xl border border-purple-200/70 bg-white/75 px-3 py-2.5 text-left transition-colors hover:bg-white active:bg-white/80"><div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl bg-purple-100"><Brain className="h-3.5 w-3.5 text-purple-600" /></div><div className="min-w-0 flex-1"><p className="text-[11px] font-bold text-foreground">Latest Eating Analysis <span className={`ml-1 text-[10px] ${scoreInfo.color}`}>{scoreInfo.label}</span></p><p className="mt-0.5 truncate text-[10px] text-muted">{latestAnalysis.report.scoreSummary}</p></div><ChevronRight className="h-4 w-4 text-muted-light transition-transform group-hover:translate-x-0.5" /></button>; })()}
      </section>
      {recommendations && <div className="fixed inset-0 z-[100] flex items-end bg-black/40" onClick={() => setRecommendations(null)}><div className="w-full rounded-t-3xl bg-card p-4" onClick={(e) => e.stopPropagation()}><div className="mb-3 flex items-center justify-between"><p className="font-extrabold">Ideas for {inferredMealType}</p><button onClick={() => setRecommendations(null)}><X className="h-5 w-5 text-muted" /></button></div>{recommendations.length === 0 ? <p className="rounded-2xl bg-background p-4 text-sm text-muted">Keep logging a few more {inferredMealType}s and I’ll learn your pattern.</p> : <div className="space-y-2">{recommendations.slice(0, 3).map((item) => <article key={item.id} className="rounded-2xl border border-border bg-background p-3"><div className="flex items-center gap-2"><span className="rounded-full bg-accent-light px-2 py-0.5 text-[9px] font-bold uppercase text-accent-dim">{item.source === "history" ? "Familiar" : "Similar idea"}</span><h3 className="text-xs font-extrabold">{item.title}</h3></div><p className="mt-1 text-[11px] text-muted">{item.source === "similar" ? `Inspired by your ${item.mealType} pattern. ${item.reason}` : `${item.mealType[0].toUpperCase() + item.mealType.slice(1)} · last had ${item.lastHadDaysAgo} days ago.`}</p>{item.adjustments.length > 0 && <p className="mt-1 text-[10px] text-accent-dim">Try: {item.adjustments.join(" · ")}</p>}</article>)}</div>}</div></div>}
    </>
  );

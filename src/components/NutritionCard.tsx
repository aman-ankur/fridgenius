    ...
    <div className="rounded-2xl border border-border bg-surface p-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {dish.hindi && <p className="text-xs text-foreground/45 mt-0.5">{dish.hindi}</p>}
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        {metrics.map((metric) => {
          ...
          return (
            <div key={metric.key} className="rounded-xl border border-border/60 bg-background/50 px-3 py-2">
              <div className="flex items-center gap-1.5 text-[10px] text-foreground/35">
                <Icon className={`h-3.5 w-3.5 ${metric.iconClass}`} />
                {metric.label}
              </div>
              <p className="mt-1 text-sm font-semibold text-foreground/85">{metric.value}</p>
            </div>
          );
        })}
      </div>
      ...
      {dish.ingredients.length > 0 && (
        <div className="mt-3">
          <p className="text-[10px] uppercase tracking-wider text-foreground/30">Key Ingredients</p>
          <div className="mt-1.5 flex flex-wrap gap-1.5">
            {dish.ingredients.map((ingredient) => (
              <span
                key={ingredient}
                className="rounded-full border border-border bg-background/60 px-2 py-1 text-[10px] text-foreground/60"
              >
                {ingredient}
              </span>
            ))}
      ...
      {dish.healthTip && (
        <p className="mt-3 rounded-lg border border-orange/20 bg-orange/5 px-3 py-2 text-xs text-foreground/65">
          {dish.healthTip}
        </p>
      )}
  ...
  return (
    <div className="mt-2">
      <button
        ...
        className="flex items-center gap-1.5 text-[10px] text-foreground/35 hover:text-foreground/55 transition-colors"
      >
      ...
      {isOpen && (
        <p className="mt-1.5 rounded-lg border border-border/60 bg-background/50 px-3 py-2 text-[10px] text-foreground/50 leading-relaxed">
          {reasoning}
        </p>
      )}
    </div>
